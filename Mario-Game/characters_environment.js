...
console.log("pulsox = "+ pulsox+ " pulsoy = "+ pulsoy)
...
function changeGameStatud(character){
  if(pulsox != "" && gameConfig.status === "start" && gamestatus == "start") {
    world_start.play();
...
function manualControl(character){
  ...
  if(character.live){
    if(pulsox <300){ // indo para a esquerda
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    if(pulsox >300){ // indo para a direita
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character){
	if(( pulsoy< 400 &&character.live) || (touchIsDown&&character.live)  ){
